...
def handle_data(self, line):
    """serial port parser for handling data from the Arduino
        This function is passed to the ArduinoManager class to handle incoming data and used like a method of ArduinoManager.
        This could be within the ArduinoManager class in serial_thread_functions_proxy.py but the thought
        is to keep it seperate and have one for each specific type of experiment. In this case, the function handles data from a
        2AFC task.
        # Inputs:
        #       self              - self reference to the ArduinoManager instance
        #       line              - incoming data from the Arduino
