function [param,stat,x_threshold]=sigm_fit(x,y,fixed_params,initial_params,plot_flag,color,threshold,ax)

% Edited by APY- 2020/2021
% Edited by JS - 2022 (added input for target axis)
...
if plot_flag==1 
    x_vector=min(x):(max(x)-min(x))/1000:max(x); %change back to 100
    %plot(x,y,'ko',x_vector,f(param(isnan(fixed_params)),x_vector),'r-')
    if isempty(ax)
        plot(x,y,'LineStyle','none','Marker','o','MarkerSize',3,'MarkerFaceColor',color,'MarkerEdgeColor',color); hold on;
        plot(x_vector,f(param(isnan(fixed_params)),x_vector),'LineStyle','-','LineWidth', 1.5,'Color',color); hold off;
        xlim([min(x) max(x)])
    else
        plot(ax,x,y,'LineStyle','none','Marker','o','MarkerSize',3,'MarkerFaceColor',color,'MarkerEdgeColor',color); hold(ax,'on');
        plot(ax,x_vector,f(param(isnan(fixed_params)),x_vector),'LineStyle','-','Color',color); hold(ax,'off');
        xlim(ax,[min(x) max(x)])    
    end
    [index,num] = rFindNearest(f(param(isnan(fixed_params)),x_vector),threshold); x_threshold = x_vector(index);
end
